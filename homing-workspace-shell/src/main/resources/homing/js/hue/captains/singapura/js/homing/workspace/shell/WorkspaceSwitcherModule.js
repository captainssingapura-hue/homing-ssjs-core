...
//
//   new WorkspaceSwitcher({ workspaceKind, workspaceTitle, group,
...
class WorkspaceSwitcher {
    ...
    constructor(opts) {
        ...
        // RFC 0058 — the group the page is in, as served: its kinds carry the
        // section slugs the anchor is minted from. A kind change is an anchor
        // on this same address; nothing here knows about /goto any more.
        this._group    = opts.group || { id: null, kinds: [] };
        this._kinds    = Array.isArray(this._group.kinds) ? this._group.kinds : [];
    ...
    _go(newTab) {
        ...
        var url = targetUrl(HrefManagerInstance.current(), { kinds: this._kinds, groupId: this._group.id, kind: c.kind, instanceId: c.instance });
    ...
    _create(name) {
        ...
        HrefManagerInstance.navigate(targetUrl(HrefManagerInstance.current(), { kinds: this._kinds, groupId: this._group.id, kind: c.kind, name: name }));
    }
