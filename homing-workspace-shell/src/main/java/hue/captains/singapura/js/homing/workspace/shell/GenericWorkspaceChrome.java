...
public final class GenericWorkspaceChrome
    ...
    protected List<String> bodyJs() {
        ...
        return List.of(
                ...
                "    // The STAMPED params only — what the server vouched for. On the flat route",
                "    // that is what the codec decoded (a legacy ws_kind included); on a path",
                "    // route it is what the placement wrote, ws_group alone. The query never",
                "    // reaches here: ?ws_kind=x on an authentic address names nothing (RFC 0057),",
                "    // and the anchor is the one way to name a kind on it (RFC 0058).",
                "    const stamped = (page && page.params) || {};",
                "    const wsGroup = stamped.ws_group || '';",
                "    const wsKind  = stamped.ws_kind  || '';",
                "    // The group: named, or the one holding a legacy kind, or a group of one.",
                "    const groupOf = function (k) { return Object.values(GROUPS).find(function (g) { return g.kinds.some(function (x) { return x.kind === k; }); }) || null; };",
                "    let group = GROUPS[wsGroup] || (wsKind ? groupOf(wsKind) : null);",
