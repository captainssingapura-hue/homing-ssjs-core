...
/** RFC 0044 — Crate conformance for {@code homing-studio-workspace}. */
class StudioWorkspaceCrateConformanceTest {

    @Test
    void everyServedModuleIsCrated() {
        assertEquals(List.of(), OrphanCheck.check(StudioWorkspaceCrate.INSTANCE),
                "every served JS module in homing-studio-workspace must be declared in its crate");
    }

    @Test
    void importsRespectCrateBoundaries() {
        assertEquals(List.of(), CrateDependencyRule.check(StudioWorkspaceCrate.INSTANCE),
