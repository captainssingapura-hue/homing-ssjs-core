...
public final class SummaryWidget extends WorkspaceWidget<WorkspaceWidget._None, SummaryWidget> {
    ...
    protected List<String> constructBodyJs() {
        ...
        return List.of(
                ...
                "    // regular MPA mode. We carry only the node's leveled TREE PATH",
                "    // (its structural position), never a stamped id. The path is",
                "    // encoded as /open?l0=..&l1=..&lN=.. and the server walks the",
                "    // same tree by that path to the doc's own url() and redirects.",
                ...
                "        var q = __openPath.map(function (p, i) {",
                "            return 'l' + i + '=' + encodeURIComponent(p);",
                "        }).join('&');",
                "        // (treeId is a future parameter for multi-tree addressing.)",
                "        window.open('/open?' + q, '_blank', 'noopener');",
                "    });",
                "",
