...
public final class SummaryWidget extends WorkspaceWidget<WorkspaceWidget._None, SummaryWidget> {
    ...
    protected List<String> constructBodyJs() {
        ...
        return List.of(
                ...
                "",
                "    // Open button — opens the selected leaf in a standalone page,",
                "    // addressed by the node's leveled TREE PATH (its structural",
                "    // position), never a stamped id (RFC 0040). EVERY leaf opens",
                "    // through the same SingleWidgetWorkspace shell:",
                "    //   ?app=singleWidgetWorkspace&widget=<w>&l0=..&l1=..",
                "    // so the page URL IS the tree path and the breadcrumb (fetched",
                "    // from /open-refs by the same path) matches it. The widget is",
                "    // chosen by kind: svg/composed render in place; every other kind",
                "    // routes to the redirect widget, which client-redirects to the",
                ...
                "        if (!__openPath) return;",
                "        var pathQ = __openPath.map(function (p, i) {",
                "            return 'l' + i + '=' + encodeURIComponent(p);",
                "        }).join('&');",
                "        var widget = WIDGET_FOR_KIND[__openKind] || 'legacy-redirect';",
                "        var url = '/app?app=singleWidgetWorkspace&widget='",
                "            + encodeURIComponent(widget) + '&' + pathQ;",
                ...
                "        // A doc leaf is any non-catalogue node; it opens by its tree",
                "        // path. Branch nodes (kind 'catalogue') have no doc page.",
                "        var isDocLeaf = node.kind && node.kind !== 'catalogue' && node.path;",
                "        __openPath = isDocLeaf ? node.path : null;",
                "        __openKind = isDocLeaf ? node.kind : null;",
                "        openBtn.style.display = __openPath ? 'inline-block' : 'none';",
                "    }",
                "    render(null);",
                "",
