...
public final class DocContentWidget extends WorkspaceWidget<WorkspaceWidget._None, DocContentWidget> {
    ...
    protected List<String> constructBodyJs() {
        ...
        return List.of(
                ...
                "    var __renderBr = null;",
                "    var __docView  = null;",   // renderDocTree handle: its dispose() reaps the scroll-spy + listener
                "    var __seq      = 0;",
                "",
                "    function tearDown() {",
                "        if (empty.parentNode) root.removeChild(empty);",
                "        // RFC 0043: dispose() reaps the IntersectionObserver + scroll-container",
                "        // listener — NOT DOM, so branch.dissolve() (DOM only) cannot. Behaviour",
                "        // teardown first, then DOM teardown; the two substrates stay separate.",
                "        if (__docView) { try { __docView.dispose(); } catch (e) {} __docView = null; }",
                ...
                "            return;",
                "        }",
                "        // RFC 0049 — claim the deep selection: an OpenDoc is a deliberate",
                "        // open (Enter / double-click in the Navigator), so this pane asks",
                "        // to be entered. The widget is inert (covered) at this moment —",
                "        // a bubbling CustomEvent still escapes an inert subtree — and the",
                "        // coordinator releases the Navigator, un-inerts this tab, and",
                "        // gives it the keyboard. No onGranted: the doc is not rendered",
                "        // yet, so the default landing (the tab content) is right; the",
                "        // async render below then autoFocuses the doc layout.",
                "        try {",
                "            root.dispatchEvent(new CustomEvent('intendedFocusIn', { bubbles: true }));",
                "        } catch (e) {}",
                "        tearDown();",
                ...
                "                    expandDepth:   99,",
                "                    // RFC 0043 + 0049 — put the keyboard on the doc layout so",
                "                    // arrows drive the TOC immediately. If the user left this",
                "                    // pane during the fetch, the tab is inert again and this",
                "                    // focus is a silent no-op — the race is safe by construction.",
                "                    autoFocus:     true",
                "                });",
                "            })",
                "            .catch(function (err) {",
