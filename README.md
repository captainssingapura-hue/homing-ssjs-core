...
**Latest release: [0.3.0 — The Workspace Substrate](https://github.com/captainssingapura-hue/japjs/releases/tag/0.3.0).** ([full notes](release-notes/0_3_0.md)) The workspace itself becomes a substrate. New `homing-workspace-shell` module hosts a constructor-injected orchestrator: `WorkspaceStateModel` folds typed events (spawn / move / close / split / merge / ratio / active) into pure state; `ReplayEngine` is a pure reducer; `CheckpointService` snapshots through a Web Worker with main-thread fallback; `WriteLockGuard` fences single-writer via Web Locks. Virtual replay supersedes physical replay — refresh and theme-change no longer re-execute the journal; the model projects onto MultiTabPane once. New modules `homing-codec` + `homing-codec-ecma` (codec generation), `homing-workspace-codecs` (typed payloads), `homing-workspace-persistence` (IndexedDB event log + checkpoint store), `homing-tree-views` (tree rendering substrate). HTML export gains an `includeChrome` toggle: when off, `data-export-chrome` is stripped and `data-export-content` elements are lifted into a fresh centred `<main>` for a standalone-doc look. RFCs 0030 – 0037 filed; new doctrines *A Good Design Doesn't Pick Up Its Substrate*, *Diligent Primitives*, *Diligent Secretaries*, *Make It Impossible*, *Lambdas Are Functional Object Shortcuts*. New conformance tests: `NoDomDestructionConformanceTest`, `EventManagementConformanceTest`, `WorkspaceCatalogueConformanceTest`, `CheckpointWorkerConformanceTest`. No breaking changes at the framework API level.

Past releases: [0.2.0 — Workspace Primitives](https://github.com/captainssingapura-hue/japjs/releases/tag/0.2.0) · [0.1.0 — The Widget Substrate](https://github.com/captainssingapura-hue/japjs/releases/tag/0.1.0). Full notes under [`release-notes/`](release-notes/).
