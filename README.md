...
**Latest release: [0.5.0 — The Rigid-Tree Document](https://github.com/captainssingapura-hue/japjs/releases/tag/0.5.0).** ([full notes](release-notes/0_5_0.md)) 0.4.0 made the *workspace* a tree-browsing product; 0.5.0 makes the *document* a rigid tree too. **RFC 0040** generalizes the tree substrate into one normalized, explicitly-leveled shape (`TreeLevel L0…L18`, `NormalizedNode`, a context-free `TreeNormalizer` per source, and `graft` — a pure recursive level shift); `CatalogueTreeAdapter` becomes the first normalizer and descends `OfStudio` portals, so a multi-studio umbrella renders its whole synthetic forest. **RFC 0039**'s renderer lands the rigid tree as a *foldable document view*: `ComposedDocNormalizer` maps a doc to a two-part `{structure, content}` payload over `/doc-tree`, and `renderDocTree` draws a TOC (the substrate `TreeRenderer`) that scrolls + **highlights** the body on selection and **folds the body subtree in sync** on expand/collapse, with focus-gated keyboard nav. **Leveled Open**: every catalogue leaf opens through one `SingleWidgetWorkspace` shell that late-binds the widget from the node kind; the path *is* the URL (`l0,l1,…`, no uuid) resolved by `ForestPathResolver`, so URL and breadcrumb share one source of truth (the legacy `/open` redirect retires). **RFC 0042** adds a leveled tree-builder DSL — `RigidDoc`, `ComposedDoc`'s successor — authored as `root().l1().l2()…lNbuild().build()`, one builder type per level so the **compiler** rejects a skipped level, an unclosed node, or a level past the cap; a worked demo ships in Building Blocks. **RFC 0041** lands `ComposedLeaf` (a per-node segment bundle, structure vs content). New `homing-studio-starter` module brings the workspace out of the box. One internal breaking change: `ContentProvider.content()` now returns a `ComposedLeaf` bundle (the `/doc-tree` content map keys each node to an array of segments); `ComposedDoc` authoring and the legacy `/doc` viewer are unchanged.

Past releases: [0.4.0 — The Workspace Becomes a Product](https://github.com/captainssingapura-hue/japjs/releases/tag/0.4.0) · [0.3.0 — The Workspace Substrate](https://github.com/captainssingapura-hue/japjs/releases/tag/0.3.0) · [0.2.0 — Workspace Primitives](https://github.com/captainssingapura-hue/japjs/releases/tag/0.2.0) · [0.1.0 — The Widget Substrate](https://github.com/captainssingapura-hue/japjs/releases/tag/0.1.0). Full notes under [`release-notes/`](release-notes/).
