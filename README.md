...
**Latest release: [0.1.0 — The Widget Substrate](https://github.com/captainssingapura-hue/japjs/releases/tag/0.1.0).** RFC 0024 ships end-to-end. `Widget<P, W>` is now the framework's unit of navigable composition; `StandardMPA` owns chrome (header + main + footer at L1 under root); per-Doc-kind viewers became "fake AppModules" preserving URL grammar while their insides upgrade to DomOpsParty branch ownership. The 528-line legacy `ComposedViewerRenderer.js` split into 9 focused EsModules per the Modest File Size doctrine; `ComposedDoc` gained recursive embedding via a new `ComposedSegment` variant — total cost ~60 lines of recursion logic because branch-tree ownership carries the cascade naturally. Tree breadcrumb bridge fixed the parallel-hierarchy gap. Three new architecture case studies file the durable observations. No breaking changes — legacy URLs unchanged via the fake-AppModule discipline.
