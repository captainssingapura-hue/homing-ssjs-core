...
**Latest release: [0.5.1 — The Document Pane](https://github.com/captainssingapura-hue/japjs/releases/tag/0.5.1).** ([full notes](release-notes/0_5_1.md)) A point release on 0.5.0: the rigid-tree doc viewer comes **into the workspace as a live pane**. A new `DocContentWidget` joins the Studio Workspace (📖 Document) and renders the *opened* document in place — foldable TOC + synced-highlight body — beside the Navigator. The enabling move is a **two-tier navigation vocabulary** on the tree: cheap *select* (arrow / single click) publishes `NodeSelected → NavigateTo` for the Summary pane, while intentional *open* (**Enter / double-click**, via `TreeRenderer`'s new `onActivate`) publishes `NodeOpened → OpenDoc` for the Document pane — so the expensive fetch + render is paid on intent, never on browsing. Each open dissolves the prior render's DomOpsParty sub-branch and mounts a fresh one (No DOM Destruction), with a sequence guard dropping a superseded fetch; `composed` kinds render in place, others degrade to a hint. Plus leveled-Open refinements: `ForestPathResolver` + `/open-refs` now emit a `Crumb(text, href)` trail so **breadcrumbs link** to ancestor catalogues, and a content-only HTML export keeps the full **two-pane TOC** with a `data-export-width` hint. The pane styles entirely through typed `StudioStyles` classes — no inline styles. No breaking changes — everything is additive.

Past releases: [0.5.0 — The Rigid-Tree Document](https://github.com/captainssingapura-hue/japjs/releases/tag/0.5.0) · [0.4.0 — The Workspace Becomes a Product](https://github.com/captainssingapura-hue/japjs/releases/tag/0.4.0) · [0.3.0 — The Workspace Substrate](https://github.com/captainssingapura-hue/japjs/releases/tag/0.3.0) · [0.2.0 — Workspace Primitives](https://github.com/captainssingapura-hue/japjs/releases/tag/0.2.0) · [0.1.0 — The Widget Substrate](https://github.com/captainssingapura-hue/japjs/releases/tag/0.1.0). Full notes under [`release-notes/`](release-notes/).
