...
public final class ComposedWidget extends DocWidget<ComposedWidget.Params, ComposedWidget> {
    ...
    protected List<String> bodyJs() {
        return List.of(
                ...
                "        errEl.textContent = 'No composed-doc id supplied. Use ?id=<uuid>.';",
                ...
                "        return 'composedWidget:' + params.id;",
                "    } });",
                "",
                "    // Cycle-detection stack threading through recursive composed segments.",
                "    var renderingStack = (params.__renderingStack || []).concat([params.id]);",
                "",
                "    // ── Two-column layout ──",
