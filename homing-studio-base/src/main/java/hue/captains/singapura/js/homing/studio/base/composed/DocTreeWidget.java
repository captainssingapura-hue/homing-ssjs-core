...
public final class DocTreeWidget extends DocWidget<DocTreeWidget.Params, DocTreeWidget> {
    ...
    protected List<String> bodyJs() {
        return List.of(
                ...
                "            loading.remove();",
                "            var __docView = renderDocTree({",
                "                branch:        bodyBranch,",
                "                container:     bodyHost,",
                "                payload:       payload,",
                "                renderContent: renderContent,",
                "                renderCaption: renderCaption,",
                "                autoFocus:     true,",   // standalone page: the doc IS the page, so grab keyboard focus on mount
                "                expandDepth:   99",
                "            });",
                "            // RFC 0043: this standalone page renders once and has no re-render",
                "            // seam (DocWidget mounts once), so the two-way-sync's observer +",
                "            // scroll listener live for the page's lifetime. dispose() on pagehide",
                "            // reaps them at the one teardown a standalone page actually has.",
