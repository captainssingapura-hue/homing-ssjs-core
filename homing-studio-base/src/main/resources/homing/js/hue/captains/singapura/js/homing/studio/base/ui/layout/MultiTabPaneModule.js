...
var _STYLE_CSS = [
    ".hmtp-leaf{display:flex;flex-direction:column;height:100%;position:relative;background:#fff;}",
    ".hmtp-strip{display:flex;align-items:center;gap:2px;padding:4px;background:#ececec;",
    "  border-bottom:1px solid #ccc;overflow-x:auto;flex-shrink:0;}",
    ...
    ".hmtp-chip-active{background:#fff;border-color:#bbb;}",
    ".hmtp-chip-close{cursor:pointer;opacity:0.5;padding:0 2px;}",
    ".hmtp-chip-close:hover{opacity:1;color:#c00;}",
    ".hmtp-pill{margin-left:auto;padding:2px 8px;background:#fff;border:1px solid #ccc;",
    "  border-radius:10px;font:11px sans-serif;color:#666;flex-shrink:0;}",
    ".hmtp-pill-full{color:#c00;border-color:#c00;}",
    ".hmtp-content{flex:1;min-height:0;position:relative;overflow:auto;}",
    ".hmtp-empty{display:flex;align-items:center;justify-content:center;color:#aaa;",
    "  font:14px sans-serif;height:100%;}",
    ...
    "  font:14px sans-serif;line-height:1;}",
    ".hmtp-cbtn:hover:not(:disabled){background:rgba(0,0,0,0.08);}",
...
class MultiTabPane {
    ...
    split(slotId, orientation) {
        ...
        // Mutate our own layout (sp.split would also work but we own state).
        ...
        if (_isLeaf(this._layout) && this._layout.slotId === slotId) {
            this._layout = {
                ...
                children: [
                    { pane: this._layout, ratio: 0.5 },
                    { pane: newLeaf,      ratio: 0.5 }
                ]
            };
        } else {
            var found = _findParent(this._layout, slotId);
            // found.parent.children[found.index] holds the leaf — replace pane.
    ...
    merge(slotId) {
        ...
        if (found.parent === this._layout) {
            this._layout = mergedLeaf;
        } else {
            _replaceSubtree(this._layout, found.parent, mergedLeaf);
        }
        this._refresh();
        return this;
    }
    ...
    getState() {
        ...
        return { layout: _clone(this._layout), tabs: tabs };
    }
    ...
    _refresh() {
        this._sp.setLayout(this._layout);
    ...
    _renderStrip(slotId, state) {
        ...
        state.tabs.forEach(function (tab) {
            ...
            chip.addEventListener("click", function () { self.switchTab(slotId, tab.id); });
