...
/**
 * The vocabulary of the global palette — RFC 0002-ext1's semantic tokens,
 * named for the framework rather than the studio (RFC 0065 D9, RFC 0066).
 *
 * <p>Semantic-only: every variable here is a role-named token that component
 * bodies reference via {@link CssVar#ref()} or as the literal {@code var(--…)}.
 * This is exactly the set {@link GlobalColorPalette.global_color_palette#declares()};
 * concrete values are bound per theme by each {@link GlobalColorPalette.Provision}.
 * The previous primitive layer ({@code --st-*} brand colours) was retired —
 * having two layers caused a "primitive doing double duty" class of bug, where
 * the same primitive served two semantic roles whose dark-mode requirements
 * diverged. Each role now owns its value independently, per theme, per mode.</p>
 *
 * <p>Colour, spacing and radius together for now — RFC 0066 Episode 1 keeps
 * the palette one flat node (D6); Episode 2 moves the scales to sub-modules of
 * their own beside this one.</p>
 */
public final class HomingVars {
