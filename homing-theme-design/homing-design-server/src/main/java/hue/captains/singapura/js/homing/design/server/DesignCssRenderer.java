...
import hue.captains.singapura.js.homing.design.Vocabulary;
...
/**
 * Renders a target group's sheet under a design: every registered projection
 * onto the target, resolved through the design and its extensions, as the
 * target's template filled with the design's variables plus the {@code :root}
 * bindings those variables read. A projection the design has no word for is
 * simply absent from the sheet — completeness is the deployment's build-time
 * check, not the renderer's; the renderer's job is to serve what there is.
 *
...
public final class DesignCssRenderer implements CssRenderer {
    ...
    public DesignCssRenderer(List<Design> designs, List<DesignExtension> extensions) {
    ...
    public Optional<String> render(CssGroup<?> group, String themeSlug) {
        ...
        Set<Class<? extends DesignClass<?, ?>>> required = new LinkedHashSet<>();
        for (DesignClass<?, ?> p : Vocabulary.onto(target.getClass())) {
