...
class GridBulkOps {
    ...
    /** Every selected CELL as an identity, deduped, ranges + cursor. */
    selectedCellIds() {
        var m = this._maps, seen = new Set(), out = [];
        function mark(i, j) {
            var id = m.resolve(i, j);
            if (!id) return;
            var k = id.pk + "\u0000" + id.column;
            if (!seen.has(k)) { seen.add(k); out.push({ pk: id.pk, column: id.column }); }
        }
        ...
        var cur = this._selection.cursorId();
        var at = cur ? m.locate(cur.pk, cur.column) : null;
        if (at) mark(at.i, at.j);
        return out;
    }

    /** Excel Delete: CLEAR the selected cells' contents (null to the domain). */
    clearSelected() {
        if (typeof this._adapter.update !== "function") return [];
        var self = this;
        var ids = this.selectedCellIds();
        ids.forEach(function (id) { self._adapter.update(id.pk, id.column, null); });
        return ids;
    }
